...
from bezier import bezier_interpolation
from parameterize import parameterize

from visualizer import matrix_show, experiment_show

"""
    EXPERIMENT PARAMETERS
"""

n = 6  # amount of data points
d = n  # amount of control points (Bézier curve Degree). If d<n, get best solution by LSM

rb = 100  # Bounds for random data points
...
raw_bezier = np.array([bezier_interpolation(data_points, t) for t in samples])  # data points as control points
fitted_bezier_uniform = np.array([bezier_interpolation(control_points_uniform, t) for t in samples])  # fitted control points uniform
fitted_bezier_chordal = np.array([bezier_interpolation(control_points_chordal, t) for t in samples])  # fitted control points chordal
...
plt.plot(fitted_bezier_uniform[:, 0], fitted_bezier_uniform[:, 1],
         label="Fitted Bezier Curve - Uniform",color=f1_color)
